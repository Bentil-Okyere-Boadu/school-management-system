...
import { MissingTenantContextException } from 'src/tenant/missing-tenant-context.exception';
...
export class HubtelDirectReceiveController {
  ...
  async receiveMoneyCallback(
    ...
    try {
      ...
      if (error instanceof MissingTenantContextException) {
        this.logger.warn(
          `Hubtel direct-receive callback rejected: inactive tenant schoolId=${schoolId}`,
        );
        throw new BadRequestException('Unknown ClientReference');
      }
      throw error;
