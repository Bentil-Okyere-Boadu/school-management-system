...
import {
  useGetCalendars,
  useGetTeacherStudentTopicPerformance,
} from "@/hooks/teacher";
...
function sanitizeReturnPath(raw: string | null): string | null {
  if (!raw || !raw.startsWith("/teacher/performance-analytics")) return null;
...
const StudentPerformanceDetailPage = () => {
  ...
  const { topicPerformance, isLoading } = useGetTeacherStudentTopicPerformance(
    studentId,
    academicTermId,
    subjectCatalogId,
    { enabled: paramsReady }
